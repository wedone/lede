...
| <br /> | `boot flash [bank <n>] <addr>` | 从 Flash 启动固件（`bank` 可选，默认 0） |
| <br /> | `boot linux <addr>`            | 将地址视作 Linux 内核入口地址并启动        |
| <br /> | `boot raw <addr>`              | 禁用中断，并从地址处直接执行               |
| <br /> | `boot breed <addr>`            | 将地址视作上传的 Breed bin 文件地址并直接启动 |
...
| <br />    | `env get <key>`         | 获取指定环境变量的值                                         |
| <br />    | `env set <key> <value>` | 设置/新建环境变量                                          |
| <br />    | `env unset <key>`       | 删除指定环境变量                                           |
| <br />    | `env clear`             | 清除所有环境变量                                           |
| <br />    | `env save`              | **保存**环境变量（修改后必须执行）                                |
| `envconf` | `envconf disable`       | 禁用环境变量功能                                           |
| <br />    | `envconf <addr> <size>` | 启用环境变量，指定 Flash 起始地址和大小（不得小于 `0x100`）              |
| <br />    | <br />                  | **注意**：部分型号（如 NAND 启动版、WDR6500v2）使用固定环境变量设置，此命令不可用 |
...
| <br />  | `flash [bank <n>] info`                      | 显示 Flash 详细信息   |
| <br />  | `flash [bank <n>] dump <addr> <size>`        | 显示 Flash 内的数据   |
| <br />  | `flash [bank <n>] read <addr> <dst> <size>`  | 将 Flash 数据读取到内存 |
| <br />  | `flash [bank <n>] erase <addr> <size>`       | 擦除 Flash        |
| <br />  | `flash [bank <n>] write <addr> <src> <size>` | 将内存数据写入 Flash   |
...
| <br /> | `gpio button`                   | 显示当前路由上按钮的状态      |
| <br /> | `gpio led`                      | 显示当前路由上部分 LED 的状态 |
| <br /> | `gpio get <n>`                  | 获取指定 GPIO 的电平状态   |
| <br /> | `gpio set <n> <hi\|lo>`         | 设置指定 GPIO 的电平     |
| <br /> | `gpio dir set <n> <in\|out>`    | 设置 GPIO 方向        |
| <br /> | `gpio led set <name> <on\|off>` | 设置 LED 亮灭         |
...
| <br /> | `mem crc32 <addr> <size>`                      | 计算 CRC32 校验                                        |
| <br /> | `mem read [byte\|short\|long] <addr>`          | 读取内存数值（1/2/4 字节）                                   |
| <br /> | `mem write [byte\|short\|long] <addr> <value>` | 写入内存数值                                             |
| <br /> | `mem write str <addr> <str>`                   | 将字符串写入内存（支持 C 语言编码）                                |
| <br /> | `mem copy <dst> <src> <size>`                  | 复制内存块                                              |
| <br /> | `mem fill <dst> <val> <size>`                  | 填充内存块                                              |
| <br /> | `mem compare <addr1> <addr2> <size>`           | 比较内存块                                              |
| `mdio` | `mdio list`                                    | 列出网络接口设备                                           |
| <br /> | `mdio <dev> dump <phy>`                        | 转储 MII 寄存器（0\~31）                                  |
| <br /> | `mdio <dev> read <phy> <reg>`                  | 读取 MII 寄存器                                         |
| <br /> | `mdio <dev> write <phy> <reg> <val>`           | 修改 MII 寄存器                                         |
| `spi`  | `spi list`                                     | 列出所有 SPI 设备                                        |
| <br /> | `spi [dev <slave>] <op> [<op> ...]`            | 执行 SPI 操作序列（`start`/`stop`/`read`/`write`/`speed`） |
...
| <br />   | `btntst enable <n>`  | 启用对 GPIO#n 的状态检测 |
| <br />   | `btntst disable <n>` | 禁用对 GPIO#n 的状态检测 |
| <br />   | `btntst restore`     | 恢复默认检测设置         |
...
1. 按下或松开路由器上的按钮，控制台会实时输出 GPIO 状态变化：
