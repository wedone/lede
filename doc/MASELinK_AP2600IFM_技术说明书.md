...
| **本机 SN**  | `0001102150865`（hwinfo 偏移 0x18 起 ASCII）                                |
...
| hwinfo     | 0xfe0000 | 0x010000 | 硬件信息：MAC\@0x0（本机 `00:27:1d:04:b6:ad`）+ SN\@0x18（ASCII）。**正常刷机流程不会触碰本分区**（RedBoot→Breed→OpenWrt 的擦写范围均不覆盖 0xfe0000），保持出厂原始值即可 |
| u-boot-env | 0xff0000 | 0x010000 | Breed 环境                                                                                                                     |

> **备注（非通用场景）**：hwinfo 分区仅在**误刷了他人编程器整片备份固件**时才会与本机不符（此时 MAC/SN 是别人的）。正常 RedBoot→Breed→OpenWrt 刷机流程永不触碰该分区，**无需也不应修改**。若确需修正（仅此场景），走 Breed 串口/Web 将正确的 board-config 写入 0xfe0000 即可（详见《迁移开发文档》§"hwinfo 还原"）。系统内 `mtd write hwinfo` 实测无效（DTS 标 read-only，内核拒绝写）。                                                                                                |
...
若设备仍为**官方固件（RedBoot 引导）**，按以下步骤在 RedBoot 环境下写入 Breed（2026-08-29 实测成功；曾用串口自动化脚本 RedBoot HTTP 加载 / fis 写入实现，现仅保留下述手动流程）。
...
| `798afeb44` | **LAN MAC 固定修复**（eth0 绑定 hwinfo\@0x0 真实 MAC）+ **救急模式执行权限修复**（rescue/reset 脚本 git 644→755，`[ -x ]` 判断此前恒失败）。另：本机 hwinfo 已于 2026-09-02 用 Breed 从社区备份 MAC 还原为本机原始数据（MAC `00:27:1d:04:b6:ad` / SN `0001102150865`，源文件 `E:\A-硬件相关\router\IP1001+ar7161+AR9220+AR9223\board-configmtd4.bin`，属特殊场景，非刷机通用操作） |
| `948b2ba1e` | docs：记录本机真实 MAC/SN 还原过程与 `798afeb44` 修复内容                                                                                                                                                                                                                                                            |
| `cdde8bd69` | **WIFI 灯改 netdev link+tx+rx 组合**（闲时常亮+数据闪烁，以 wlan 接口 link 为准）+ **救急模式 eth0 闪断 1s**（免拔插网线强制电脑重发 DHCP）                                                                                                                                                                                                 |
| `c60c5e692` | **MAC 绑定最终修正**：hwinfo nvmem 声明改官方 AP-105 同款 `nvmem-cells` 内联写法（原 `nvmem-layout/fixed-layout` 依赖未编译内核驱动，致 MAC 随机；本固件实测 MAC 固定本机真实值）。此版本已建 GitHub Release                                                                                                                                              |
| `e90628eb4` | **CI 产物防误用**：编译产物与 artifact 名带 commit 短 hash（避免固定文件名误烧旧固件），配套 `download_latest.py`/`serve_latest.py` 自动定位最新产物并 sha256 核对                                                                                                                                                                             |
