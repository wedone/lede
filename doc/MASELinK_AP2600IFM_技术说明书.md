...
2. 救急模式激活：LAN 上临时增加辅助 IP `192.168.9.1/24` + 独立 DHCP（.100-.200）；
3. 电脑插网线到 LAN，**自动获取到 192.168.9.x 地址**；
4. 浏览器访问 **<http://192.168.9.1>** → 进入 LuCI 管理页（原有 192.168.3.1 仍有效）。

### 5.3 特性与恢复

* **纯内存态**：不写入 flash/overlay，**重启后自动消失**，不影响原配置

* 救急期间原有 IP（如 192.168.3.1）**仍然可用**，二者共存
...
```
辅助 IP  192.168.9.1/24  -> ip addr add 叠加到 br-lan
临时 DHCP 192.168.9.100-200 -> 独立 dnsmasq 实例(/tmp/rescue-dnsmasq.conf)
标记文件 /tmp/rescue-mode
```
...
1. 检查电脑 IP：若取到 192.168.9.x → 说明救急模式已激活，用 **192.168.9.1** 访问
...
* **长按按钮约 5 秒进入救急模式** → 192.168.9.1 管理，或 SSH 修复
