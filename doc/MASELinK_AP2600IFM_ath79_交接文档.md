...
| `.github/workflows/build-ap2600ifm.yml`                | 专用 workflow：push ap2600 / workflow\_dispatch 触发，产物上传      |
...
* **LED（6 个，active\_low）**：GPIO 0=d24 / 2=rf1 / 3=d24top / 4=rf2 / 5=rf2top / 7=rf1top

* **复位键**：GPIO 8，active\_low
...
* 硬件参数**唯一权威**是旧 `mach-maselink-ap2600ifm.c`（作者 Weijie Gao = Breed 作者）；**严禁参考 aruba\_ap-105**（硬件无关）

* Web 访问 GitHub Actions：<https://github.com/wedone/lede/actions（需登录> wedone 账号，或直接用 `gh` CLI）

* 重新构建触发方式：改代码后 `git push origin ap2600`，或 web 端 workflow\_dispatch
...
* **修复（本次待提交）**：`generic.mk LOADER_FLASH_OFFS=0x52000`（已改）+ `config.h` 中
  ...
  * workflow 强制清理 lzma-loader 缓存（`make target/linux/clean`、删 build\_dir 下 lzma-loader）确保重编。
